...
| 📊 **个人答题记录** | 从小红书或 B 站小程序进入时，自动识别当前用户；题库标记已玩、已解出，记录页显示完成数与公开题目总数。 |
| ✍️ **玩家投稿** | 觉得意犹未尽？在「出题」写下汤面、汤底和提示，提交后进审核队列。 |
| 🛡 **管理员后台** | 访问 `/admin` 先通过浏览器原生账号密码验证，再审核、发布、驳回或删除投稿；操作有日志可查。 |
| 🔗 **一键分享** | 一道题生成可分享链接，朋友的浏览器里直接开局。 |
...
- **汤底保密**：公开题库接口从来不返回答案，只有玩家点了「公布答案」才单独取一次。
- **置信度阈值** (`JEV_CONFIDENCE_THRESHOLD = 0.4`)：低于阈值的判断一律改写为「无法确定」，宁可让 Jev 承认拿不准，也不让它猜错误导玩家。
- **身份无感**：普通网页访客共用 `web-shared` ID；小红书、B 站小程序使用各自的 `open_id` 建立独立用户 ID，不显示注册 / 登录页。平台账号之间暂不自动合并。
- **答题记录**：平台用户的有效提问和真相还原写入 D1；Jev 判定「破解成功」且置信度达到 0.4 时标记完成。普通网页访客不保存个人答题记录。
- **题库双源**：内置题库写在 `data/library.json`，worker 启动时整段覆盖种子行；玩家投稿存在 D1，由管理员发布后才进入推荐题库。
