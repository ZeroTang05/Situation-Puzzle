...
- **题库双源**：内置题库写在 `data/library.json`，Worker 启动时覆盖种子行；玩家投稿经 Jev 二元审核后写入 D1，通过后排在题库末端，管理员仍可复核。
- **中英切换**：页面右上角的 `EN / ZH` 按钮切换语言，选择会保存在当前浏览器。30 道内置题在 `data/library.en.json` 中有完整英文汤面、汤底和提示，两种语言沿用相同题目 ID 与答题记录。玩家投稿按提交时的语言进入对应题库；Jev 的审核、提问和还原真相也使用该语言的输入、选项与规则。
